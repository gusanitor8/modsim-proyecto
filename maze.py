...
class MazeGenerator:
    ...
    def __init__(self, width=21, height=21):
        ...
        self.width = width
        self.height = height
        self.maze = [[self.WALL for _ in range(width)] for _ in range(height)]
        self.entry = (1, 1)
    ...
    def carve_passages_from(self, x, y):
        ...
        for dx, dy in self.DIRECTIONS:
            ...
            nx, ny = x + dx, y + dy
            if self.is_within_bounds(nx, ny) and self.maze[ny][nx] == self.WALL:
                # Carve a path between the current cell and the next cell
                self.maze[y + dy // 2][x + dx // 2] = self.PATH
